...
int main(){
    ...
    bool flag=false;
    int row=0, colm=m-1;
    while(row<n && colm>=0){     /// optimized solution for thst matrix must be sorted.
        if(array[row][colm]==key){
            flag=true;
            //break;            
        }
        array[row][colm]>key ? colm--:row++; /// another method to eliminate else . 
        // else if(array[row][colm]>key){
        //     colm--;
        // }
        // else{
        //     row++;
